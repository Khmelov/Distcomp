...
class CassandraPostRepository:
    ...
    async def create(self, tweet_id: int, content: str, state: str) -> dict:
        new_id = uuid.uuid1().int & 0xFFFFFFFFFFFFFFFF
        stmt = SimpleStatement("INSERT INTO tbl_post (tweet_id, id, content, state) VALUES (%s, %s, %s, %s)")
        self.session.execute(stmt, (tweet_id, new_id, content, state))
        return {"id": new_id, "tweet_id": tweet_id, "content": content, "state": state}
