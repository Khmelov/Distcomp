...
use crate::service::v1::ServiceState;
...
pub async fn create_topic(
    ...
    match state.topic_storage.create(topic).await {
        Ok(topic) => (
            StatusCode::CREATED,
...
pub async fn list_stories(State(state): State<ServiceState>) -> impl IntoResponse {
    match state.topic_storage.list().await {
        Ok(stories) => {
            let dtos: Vec<TopicResponseTo> =
                stories.into_iter().map(TopicResponseTo::from).collect();
...
pub async fn get_topic(
    ...
    match state.topic_storage.get(id).await {
...
pub async fn update_topic_id(
    ...
    match topic.validate() {
        Ok(()) => {
            let res = state.topic_storage.update(id, topic).await.unwrap();
...
pub async fn delete_topic(
    ...
    match state.topic_storage.delete(id).await {
