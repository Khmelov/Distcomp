...
use serde_json::json;
use storage::cassandra::notice::CassandraNoticeRepo;
use tokio_stream::StreamExt;
use rdkafka::message::Message;

use kafka_types::{KafkaRequest, KafkaResponse};
use service::notice::{
    do_create_notice, do_delete_notice, do_get_notice, do_list_notices, do_update_notice,
};
    ...
    let (status, headers, body) = match (method, path) {
        ("POST", "/api/v1.0/notices") => {
            if let Some(val) = parse_json(&body_bytes) {
                match serde_json::from_value::<common::NoticeRequestTo>(val) {
                    Ok(payload) => match do_create_notice(state, payload).await {
                        Ok(dto) => (201, vec![("content-type".into(), "application/json".into())], serde_json::to_vec(&dto).ok()),
                        Err((code, msg)) => (code.as_u16(), vec![], serde_json::to_vec(&json!({ "error": msg })).ok()),
                    },
        ...
        ("GET", "/api/v1.0/notices") => match do_list_notices(state).await {
            Ok(dtos) => (200, vec![("content-type".into(), "application/json".into())], serde_json::to_vec(&dtos).ok()),
            Err(code) => (code.as_u16(), vec![], None),
        },
        ("GET", path) if path.starts_with("/api/v1.0/notices/") => {
            let id_str = &path["/api/v1.0/notices/".len()..];
            if let Ok(id) = id_str.parse::<domain::entities::IDType>() {
                match do_get_notice(state, id).await {
                    Ok(Some(dto)) => (200, vec![("content-type".into(), "application/json".into())], serde_json::to_vec(&dto).ok()),
        ...
        ("PUT", path) if path.starts_with("/api/v1.0/notices/") => {
            let id_str = &path["/api/v1.0/notices/".len()..];
            if let Ok(id) = id_str.parse::<domain::entities::IDType>() {
                if let Some(val) = parse_json(&body_bytes) {
                    match serde_json::from_value::<common::NoticeRequestTo>(val) {
                        Ok(payload) => match do_update_notice(state, id, payload).await {
                            Ok(dto) => (200, vec![("content-type".into(), "application/json".into())], serde_json::to_vec(&dto).ok()),
                            Err((code, msg)) => (code.as_u16(), vec![], serde_json::to_vec(&json!({ "error": msg })).ok()),
                        },
                        Err(_) => (400, vec![], Some(b"Invalid JSON".to_vec())),
                    }
        ...
        ("DELETE", path) if path.starts_with("/api/v1.0/notices/") => {
            let id_str = &path["/api/v1.0/notices/".len()..];
            if let Ok(id) = id_str.parse::<domain::entities::IDType>() {
                match do_delete_notice(state, id).await {
                    Ok(()) => (204, vec![], None),
                    Err(code) => (code.as_u16(), vec![], None),
                }
            } else {
...
async fn main() {
    ...
    let producer: FutureProducer = ClientConfig::new()
        ...
        .create().expect("Failed to create Kafka producer");

    let consumer: StreamConsumer = ClientConfig::new()
        .set("bootstrap.servers", "localhost:9092")
        .set("group.id", "notices_backend_group")
        .set("auto.offset.reset", "earliest")
        .create().expect("Failed to create Kafka consumer");
    consumer.subscribe(&[KAFKA_REQUESTS_TOPIC]).expect("Failed to subscribe");
    ...
    let listener = tokio::net::TcpListener::bind("0.0.0.0:24130").await.unwrap();
