...
const KAFKA_RESPONSE_TOPIC: &str = "service_responses";
...
async fn proxy_request(
        ...
        .filter_map(|(name, value)| {
            ...
            if matches!(
                ...
                "host" | "content-length" | "transfer-encoding" | "connection"
                    | "upgrade" | "http2-settings" | "keep-alive" | "proxy-connection" | "expect"
            ) {
                None
            } else {
    ...
    match state.kafka_producer.send(record, Duration::from_secs(5)).await {
        ...
        Err((e, _)) => {
            let mut pending = state.pending_responses.lock().await;
            pending.remove(&cid);
            return (StatusCode::BAD_GATEWAY, format!("Kafka produce error: {}", e)).into_response();
        }
    };
    ...
    let kafka_resp = match tokio::time::timeout(
        ...
        Ok(Err(_)) => {
            let mut pending = state.pending_responses.lock().await;
            pending.remove(&cid);
            return (StatusCode::BAD_GATEWAY, "Internal error: response channel closed").into_response();
        }
        Err(_) => {
            // таймаут
            let mut pending = state.pending_responses.lock().await;
    ...
    let status = StatusCode::from_u16(kafka_resp.status)
...
pub async fn create_notice(
    ...
    proxy_request(&state, parts.method, "/api/v1.0/notices".to_string(), parts.headers, body).await
}
...
pub async fn get_notice(
    ...
    let path = format!("/api/v1.0/notices/{}", id);
    let (parts, body) = req.into_parts();
