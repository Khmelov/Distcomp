...
use domain::entities::{IDType, topic::*};

#[derive(Clone)]
pub struct InMemoryTopicRepo {
    store: Arc<DashMap<IDType, Topic>>,
...
impl InMemoryTopicRepo {
    pub fn create(&mut self, mut entity: Topic) -> Result<Topic, Error> {
    ...
    pub fn get(&self, id: IDType) -> Result<Option<Topic>, Error> {
        Ok(self.store.get(&id).map(|entity| entity.clone()))
    }
    pub fn update(&mut self, id: IDType, mut entity: Topic) -> Result<Topic, Error> {
