...
#[derive(Clone)]
pub struct CassandraNoticeRepo {
    session: Arc<Session>,
...
impl CassandraNoticeRepo {
    pub async fn new(contact_points: &str, keyspace: &str) -> Result<Self, QueryError> {
        ...
        let insert_stmt = session
            .prepare("INSERT INTO notices (id, topic_id, content) VALUES (?, ?, ?)")
            .await?;
        let select_stmt = session
            .prepare("SELECT id, topic_id, content FROM notices WHERE id = ?")
            .await?;
        let delete_stmt = session.prepare("DELETE FROM notices WHERE id = ?").await?;
        let select_all_stmt = session
            .prepare("SELECT id, topic_id, content FROM notices")
            .await?;
    ...
    pub async fn save(&self, mut entity: Notice) -> Result<Notice, QueryError> {
        if entity.id == 0 {
        ...
        self.session
            .execute(
                &self.insert_stmt,
                (entity.id, entity.topic_id, &entity.content),
            )
            .await?;
        Ok(entity)
    }

    pub async fn get(&self, id: IDType) -> Result<Option<Notice>, QueryError> {
        let rows = self
            .session
        ...
        if let Some(row) = rows.into_iter().next() {
            let notice: Notice = row.into_typed::<Notice>().unwrap();
            Ok(Some(notice))
        } else {
            Ok(None)
        }
    ...
    pub async fn update(&self, id: IDType, entity: Notice) -> Result<Notice, QueryError> {
        self.session
            .execute(&self.insert_stmt, (id, entity.topic_id, &entity.content))
            .await?;
        Ok(entity)
    }
    ...
    pub async fn list(&self) -> Result<Vec<Notice>, QueryError> {
        let rows = self
            .session
        ...
        for row in rows {
            let msg: Notice = row.into_typed::<Notice>().unwrap();
            notices.push(msg);
        }
        Ok(notices)
    }
