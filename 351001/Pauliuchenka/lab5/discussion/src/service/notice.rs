...
pub async fn do_create_notice(
    ...
    let notice: Notice = payload.try_into().map_err(|_| (StatusCode::BAD_REQUEST, "Invalid data".into()))?;
    notice.validate().map_err(|_| (StatusCode::BAD_REQUEST, "Validation failed".into()))?;
    state.notice_storage.save(notice).await
        .map(|s| s.into())
...
pub async fn do_list_notices(state: &ServiceState) -> Result<Vec<NoticeResponseTo>, StatusCode> {
    ...
    state.notice_storage.list().await
        .map(|v| v.into_iter().map(|m| m.into()).collect())
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
}

pub async fn do_get_notice(state: &ServiceState, id: IDType) -> Result<Option<NoticeResponseTo>, StatusCode> {
...
pub async fn do_update_notice(
    ...
    let notice: Notice = payload.try_into().map_err(|_| (StatusCode::BAD_REQUEST, "Invalid data".into()))?;
    notice.validate().map_err(|_| (StatusCode::BAD_REQUEST, "Validation failed".into()))?;
    if state.notice_storage.get(id).await.map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, "DB error".into()))?.is_none() {
        return Err((StatusCode::NOT_FOUND, "Not found".into()));
    }
    state.notice_storage.update(id, notice).await
        .map(|m| m.into())
...
pub async fn do_delete_notice(state: &ServiceState, id: IDType) -> Result<(), StatusCode> {
    state.notice_storage.delete(id).await.map_err(|_| StatusCode::NOT_FOUND)
}
