...
pub async fn create_label(
    ...
    let label: Label = payload.try_into().unwrap();
    match label.validate() {
        Ok(()) => match state.label_storage.create(label).await {
            Ok(label) => (
                StatusCode::CREATED,
...
pub async fn list_labels(State(state): State<ServiceState>) -> impl IntoResponse {
    match state.label_storage.list().await {
        Ok(labels) => {
...
pub async fn get_label(
    ...
    match state.label_storage.get(id).await {
...
pub async fn delete_label(
    ...
    match state.label_storage.delete(id).await {
