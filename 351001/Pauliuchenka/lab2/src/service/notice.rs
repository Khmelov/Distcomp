...
pub async fn create_notice(
    ...
    let notice: Notice = payload.try_into().unwrap();
    if notice.validate() != Ok(()) {
        return (StatusCode::BAD_REQUEST, Json(json!({})));
    }

    match state.topic_storage.read().await.get(notice.topic_id).await {
        Ok(Some(_topic)) => match state.notice_storage.write().await.create(notice).await {
            Ok(notice) => (
                StatusCode::CREATED,
...
pub async fn list_notices(State(state): State<ServiceState>) -> impl IntoResponse {
    match state.notice_storage.read().await.list().await {
        Ok(notices) => {
...
pub async fn get_notice(
    ...
    match state.notice_storage.read().await.get(id).await {
        Ok(Some(notice)) => (
            StatusCode::OK,
            Json(json!(NoticeResponseTo::from(notice))),
        ),
...
pub async fn delete_notice(
    ...
    match state.notice_storage.write().await.delete(id).await {
