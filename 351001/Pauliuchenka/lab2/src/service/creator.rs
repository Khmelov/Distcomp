...
pub async fn create_creator(
    ...
    match creator.validate() {
        Ok(()) => {
            match state.creator_storage.write().await.create(creator).await {
                Ok(creator) => (
                    StatusCode::CREATED,
...
pub async fn list_creators(State(state): State<ServiceState>) -> impl IntoResponse {
    match state.creator_storage.read().await.list().await {
        Ok(creators) => {
...
pub async fn get_creator(
    ...
    match state.creator_storage.read().await.get(id).await {
...
pub async fn delete_creator(
    ...
    match state.creator_storage.write().await.delete(id).await {
