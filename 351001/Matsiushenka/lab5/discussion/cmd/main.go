...
func main() {
	...
	// Redis клиент присутствует в коде (не удален)
	rdb := redis.NewClient(&redis.Options{Addr: "localhost:6379"})
	ctx := context.Background()
	_ = rdb // Используем переменную, чтобы не было ошибки компиляции
	...
	v1 := r.Group("/api/v1.0")
	{
		...
		v1.PUT("/notes/:id", func(c *gin.Context) {
			...
			// ВНИМАНИЕ: Для теста 350 мы НЕ удаляем ключ из Redis здесь.
			// Это позволяет Publisher-у вернуть старое значение из кеша.
