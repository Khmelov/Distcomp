...
public class KafkaCommentService : ICommentService
{
    ...
    public KafkaCommentService(IProducer<string, string> producer, IIssueRepository issueRepository, ICacheService cache)
    {
        _producer = producer;
    ...
    public async Task<CommentResponseTo> CreateAsync(CommentRequestTo request)
    {
        ...
        long generatedId = DateTime.UtcNow.Ticks & long.MaxValue;
    ...
    {
        string key = $"CommentsByIssue:{issueId}";
        
        var cachedList = await _cache.GetAsync<List<CommentResponseTo>>(key);
        if (cachedList != null) return cachedList;
        
        var response = await SendRequestAndWaitResponse("GET_BY_ISSUE", new CommentRequestTo { IssueId = issueId }, issueId.ToString());
    ...
    public async Task DeleteByIssueIdAsync(long issueId)
    {
        ...
        await _producer.ProduceAsync(KafkaTopics.InTopic, new Message<string, string> {
            ...
            Value = JsonSerializer.Serialize(new KafkaRequest { Method = "DELETE_BY_ISSUE", Payload = new CommentRequestTo { IssueId = issueId }, CorrelationId = "" })
        });
