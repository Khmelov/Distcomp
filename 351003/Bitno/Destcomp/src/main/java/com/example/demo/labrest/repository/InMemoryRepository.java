...
public class InMemoryRepository {
    ...
    public void init() {
        ...
        creator.setId(idGen.getAndIncrement());
        creator.setLogin("misabitnol@gmail.com");
        creator.setPassword("pass1234");
        creator.setFirstname("Михаил");
        creator.setLastname("Битно");
        creator.setRole("CUSTOMER");
        creators.put(creator.getId(), creator);
    }

    public Creator saveCreator(Creator c) {
        if (c.getId() == null) {
            c.setId(idGen.getAndIncrement());
        }
        creators.put(c.getId(), c);
        return c;
    }
    ...
    public Marker saveMarker(Marker m) {
        if (m.getId() == null) {
    ...
    public Creator updateCreator(Creator c) {
        creators.put(c.getId(), c);
        return c;
    }
    ...
    public Optional<Creator> getCreator(Long id) {
    ...
    public Optional<Marker> getMarker(Long id) {
