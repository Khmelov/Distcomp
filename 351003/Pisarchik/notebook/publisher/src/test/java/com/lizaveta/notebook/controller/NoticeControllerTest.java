...
import com.lizaveta.notebook.config.InMemoryDiscussionTestConfig;
...
@Import(InMemoryDiscussionTestConfig.class)
class NoticeControllerTest extends PostgresTestContainerConfig {
    ...
    @Autowired
    private DiscussionNoticeClient discussionNoticeClient;

    private Long storyId;

    @BeforeEach
    void setUp() {
        ((InMemoryDiscussionNoticeClient) discussionNoticeClient).clear();
    ...
    void getNotices_ShouldReturn200AndPage() {
        given()
                .when()
                .get("/api/v1.0/notices?sortBy=id")
                .then()
                .statusCode(200)
