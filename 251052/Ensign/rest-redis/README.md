A distributed system for managing News and Comments, refactored into a microservices architecture. It uses **PostgreSQL** for the core News/Author/Tag domain, **Cassandra** for the high-volume Comment domain, and **Redis** for distributed caching in the publisher service.

## 🏗 Architecture

The project is structured as a multi-module Maven project:

*   **`publisher`**: The main monolythic module (Rest-JPA refactored). Manages Authors, News, and Marks. Backed by PostgreSQL and utilizes **Redis** for distributed caching.
    *   Port: `24110`
...
*   **Databases**:
    ...
    *   **Redis** (Distributed Cache for Publisher service)
*   **Message Broker**:
    *   **Apache Kafka** (Asynchronous comment transport and moderation)
*   **Monitoring & Visualization**:
    *   **Redis Insight** (Redis GUI - Port `8001`)
    *   **Kafka-UI** (Port `8080`)
*   **Liquibase** (Database Migration for both Postgres and Cassandra)
*   **Docker & Docker Compose** (Infrastructure in KRaft mode)
*   **Testcontainers** (Used for integration testing with Postgres and Redis)
*   **SpringDoc OpenAPI** (Swagger UI)

## 🚀 Distributed Caching (Redis)

To improve system performance and reduce the load on the `discussion` service and primary databases, we've implemented a distributed caching layer using Redis in the `publisher` service.

### Caching Strategy
- **Authors**: Cached by ID (`authors::id`). Updated on edit, evicted on delete.
- **News**: Cached by ID (`news::id`). Includes the associated comments from the `discussion` service.
- **Marks**: Cached by ID (`marks::id`).

### Features
- **TTL**: Cache entries have a default Time-To-Live of **10 minutes**.
- **JSON Serialization**: Data is stored in Redis as JSON using `GenericJackson2JsonRedisSerializer`, supporting Java 8 date/time types and records.
- **Resilience**: Caching is non-blocking; if Redis is unavailable, the system falls back to direct database/service calls.
...
1.  **Start the Infrastructure**:
    Use Docker Compose to spin up PostgreSQL, Cassandra, Kafka, and Redis.
    ```bash
    docker-compose up -d
    ```
...
*   **Redis Insight**: [http://localhost:8001](http://localhost:8001)
    *   GUI for visualizing and managing cached data in Redis.
