...
class NoticeControllerTest {
    ...
    void createNoticeTest_shouldReturnCreated() {
        ...
        var author = restTemplate.postForEntity(AUTHORS_URL,
                new AuthorRequestToDto("noticeuser", "pass", "Notice", "User"),
                AuthorResponseToDto.class);
        assertThat(author.getBody())
                .isNotNull();
        var issue = restTemplate.postForEntity(ISSUES_URL,
                new IssueRequestToDto(author.getBody().id(), "Test Issue", "Content"),
    ...
    void getNoticeByIdTest_shouldReturnOk() {
        // Подготовка
        var author = restTemplate.postForEntity(AUTHORS_URL,
                new AuthorRequestToDto("usera", "123", "A", "B"),
                AuthorResponseToDto.class);
        assertThat(author.getBody())
                .isNotNull();
        var issue = restTemplate.postForEntity(ISSUES_URL,
                new IssueRequestToDto(author.getBody().id(), "Issue for notice", "Content"),
                IssueResponseToDto.class);
    ...
    void updateNoticeTest_shouldReturnOk() {
        // Подготовка
        var author = restTemplate.postForEntity(AUTHORS_URL,
                new AuthorRequestToDto("updater", "pass", "Updater", "N"),
                AuthorResponseToDto.class);
        assertThat(author.getBody())
                .isNotNull();
        var issue = restTemplate.postForEntity(ISSUES_URL,
                new IssueRequestToDto(author.getBody().id(), "Update issue", "Content"),
                IssueResponseToDto.class);
        assertThat(issue.getBody())
                .isNotNull();
        var notice = restTemplate.postForEntity(NOTICES_URL,
    ...
    void deleteNoticeTest_shouldReturnNoContent() {
        // Подготовка
        var author = restTemplate.postForEntity(AUTHORS_URL,
                new AuthorRequestToDto("deleter", "123", "Del", "N"),
                AuthorResponseToDto.class);
        assertThat(author.getBody())
                .isNotNull();
        var issue = restTemplate.postForEntity(ISSUES_URL,
                new IssueRequestToDto(author.getBody().id(), "Del issue", "Content"),
                IssueResponseToDto.class);
        assertThat(issue.getBody())
                .isNotNull();
        var notice = restTemplate.postForEntity(NOTICES_URL,
    ...
    void getNoticesByIssueIdTest_shouldReturnOk() {
        ...
        var author = restTemplate.postForEntity(AUTHORS_URL,
                new AuthorRequestToDto("finder", "pass", "Finder", "N"),
                AuthorResponseToDto.class);
        assertThat(author.getBody())
                .isNotNull();
        var issue = restTemplate.postForEntity(ISSUES_URL,
                new IssueRequestToDto(author.getBody().id(), "Target issue", "Content"),
                IssueResponseToDto.class);
        assertThat(issue.getBody())
                .isNotNull();
