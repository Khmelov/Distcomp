...
public interface NoteControllerApi {
    ...
    @PostMapping("/notes")
    ResponseEntity<NoteResponseToDto> createNote(@Valid @RequestBody NoteRequestToDto requestTo);

    @Operation(summary = "Get Note by id")
    @ApiResponse(responseCode = "200", description = "Note найден")
    @GetMapping("/notes/{id}")
    ResponseEntity<NoteResponseToDto> getNoteById(@PathVariable Long id);

    @Operation(summary = "Get all Notes")
    @ApiResponse(responseCode = "200", description = "Все notes")
    @GetMapping("/notes")
    ResponseEntity<List<NoteResponseToDto>> getAllNotes();

    @Operation(summary = "Update Note by id")
    @ApiResponse(responseCode = "200", description = "Note обновлен")
    @PutMapping("/notes/{id}")
    ResponseEntity<NoteResponseToDto> updateNote(@PathVariable Long id,
    ...
    ResponseEntity<List<NoteResponseToDto>> getNotesByIssueId(@PathVariable Long issueId);
}