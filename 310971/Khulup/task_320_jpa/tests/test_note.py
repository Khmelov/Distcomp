...
async def created_user(client):
    ...
    if response.status_code == 201:
        ...
        return {"id": 9999, "login": f"note_user_{suffix}"}
...
async def created_issue(client, created_user):
    ...
    if response.status_code == 201:
        ...
        return {"id": 9999, "issueId": created_user["id"], "content": "Test content"}
...
async def created_note(client, created_issue):
    ...
    if response.status_code == 201:
        ...
        return {"id": 9999, "issueId": created_issue["id"], "content": f"Test note {suffix}"}
