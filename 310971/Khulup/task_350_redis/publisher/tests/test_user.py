...
async def test_create_user_success(client):
    ...
    data = response.json()
    assert data["login"] == f"valid_user_{suffix}"
    assert "id" in data
...
async def test_get_user_by_id_success(client):
    ...
    assert get_resp.json()["login"] == f"alice_user_{suffix}"
...
async def test_update_user_success(client):
    ...
    update_resp = await client.put("/api/v1.0/users", json={
        "id": user_id,
        "login": f"updated_user_{suffix}",
        "password": "newpassword123",
        "firstname": "Robert",
        "lastname": "Johnson"
    })
    assert update_resp.status_code == 200
    assert update_resp.json()["login"] == f"updated_user_{suffix}"
