...
def get_issue(issue_id: str):
    ...
    try:
        try:
            ...
            repo = IssueRepository(session)
            service = IssueService(repo)
            issue = service.get_issue(issue_id_int)
        except ValueError:
            repo = IssueRepository(session)
            service = IssueService(repo)
            issue = None
        ...
        logging.error(f"Error getting issue {issue_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
    finally:
        session.close()
...
def list_issues(
    ...
    try:
        ...
        logging.error(f"Error listing issues: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
    finally:
        session.close()
...
def update_issue_body(data: IssueUpdate):
    ...
    try:
        ...
        logging.error(f"Error updating issue {data.id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
    finally:
        session.close()

@router.put("/{issue_id}", response_model=IssueRead)
def update_issue(issue_id: str, data: IssueCreate):
    session = SessionLocal()
    try:
        try:
            issue_id_int = int(issue_id)
        except ValueError:
            raise HTTPException(status_code=422, detail="Invalid issue ID format")
        
        repo = IssueRepository(session)
        service = IssueService(repo)
        update_data = IssueUpdate(id=issue_id_int, userId=data.userId, title=data.title, content=data.content)
...
def delete_issue(issue_id: str):
    ...
    try:
        ...
        repo = IssueRepository(session)
        service = IssueService(repo)
        success = service.delete_issue(issue_id_int)
        if not success:
            raise HTTPException(status_code=404, detail="Issue not found")
    except HTTPException:
        raise
