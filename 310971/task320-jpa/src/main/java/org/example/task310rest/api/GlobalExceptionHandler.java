...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleConflict(ConflictException ex) {
        return build(HttpStatus.FORBIDDEN, ex.getMessage(), "40301");
    }
    ...
    public ResponseEntity<ApiError> handleDataIntegrityViolation(DataIntegrityViolationException ex) {
        ...
        if (message != null && message.contains("duplicate") || message != null && message.contains("unique")) {
            return build(HttpStatus.FORBIDDEN, "Duplicate entry violation", "40302");
        }
        return build(HttpStatus.FORBIDDEN, "Data integrity violation", "40303");
    }
