...
public class TweetService {
    ...
    public TweetResponseTo getById(Long id) {
        ...
        Optional<TweetResponseTo> cached = cacheService.getTweetFromCache(id, TweetResponseTo.class);
        if (cached.isPresent()) {
            log.debug("Tweet found in cache: {}", id);
            return cached.get();
        }
        ...
        TweetResponseTo response = tweetMapper.toResponseTo(tweet);

        // Cache the result
        cacheService.cacheTweet(id, response);
    ...
    public TweetResponseTo create(TweetRequestTo requestTo) {
        ...
        TweetResponseTo response = tweetMapper.toResponseTo(saved);

        // Cache the new tweet
        cacheService.cacheTweet(saved.getId(), response);
    ...
    public TweetResponseTo update(Long id, TweetRequestTo requestTo) {
        ...
        TweetResponseTo response = tweetMapper.toResponseTo(updated);
