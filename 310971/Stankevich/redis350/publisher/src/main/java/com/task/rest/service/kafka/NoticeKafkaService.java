...
public class NoticeKafkaService {
    ...
    public NoticeResponseTo create(NoticeRequestTo request) {
        ...
        try {
            ...
            NoticeResponseTo result = convertToResponseTo(response);

            // Cache the created notice
            cacheService.cacheNotice(result.getId(), result);
            ...
            // Cache even on timeout (optimistic)
            cacheService.cacheNotice(id, response);
    ...
    public NoticeResponseTo getByIdOnly(Long id) {
        ...
        Optional<NoticeResponseTo> cached = cacheService.getNoticeFromCache(id, NoticeResponseTo.class);
        if (cached.isPresent()) {
            log.debug("Notice found in cache: {}", id);
            return cached.get();
        }
        ...
        try {
            ...
            // Cache the result
            cacheService.cacheNotice(id, result);
    ...
    public List<NoticeResponseTo> getAll() {
        ...
        try {
            ...
            if (response.getNotices() != null) {
                List<NoticeResponseTo> results = response.getNotices().stream()
                        .map(this::convertDtoToResponseTo)
                        .collect(Collectors.toList());

                // Cache all results
                results.forEach(notice -> cacheService.cacheNotice(notice.getId(), notice));
