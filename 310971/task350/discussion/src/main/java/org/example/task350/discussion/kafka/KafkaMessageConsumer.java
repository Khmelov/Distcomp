...
public class KafkaMessageConsumer {
    ...
    private KafkaMessageResponse handlePost(KafkaMessageRequest request) {
        ...
        if (moderatedState != MessageState.PENDING) {
            ...
            org.example.task350.discussion.model.Message entity = messageService.getEntityById(
    ...
    private KafkaMessageResponse handleDelete(KafkaMessageRequest request) {
        try {
            String country = request.getCountry() != null ? request.getCountry() : "default";
            Long tweetId = request.getTweetId();
            Long id = request.getId();
            
            // If tweetId is not provided, try to find message by ID first
            if (tweetId == null) {
                List<MessageResponseTo> allMessages = messageService.getAll();
                ...
                if (found == null) {
                    ...
                    response.setError("Message not found: " + id);
                    return response;
                }
                
                country = found.getCountry();
                tweetId = found.getTweetId();
            }
            
            messageService.delete(country, tweetId, id);

            KafkaMessageResponse response = new KafkaMessageResponse();
            response.setRequestId(request.getRequestId());
            response.setOperation("DELETE");
            return response;
        } catch (org.example.task350.discussion.exception.NotFoundException e) {
            KafkaMessageResponse response = new KafkaMessageResponse();
            response.setRequestId(request.getRequestId());
            response.setOperation("DELETE");
            ...
            log.error("Error in handleDelete: requestId={}", request.getRequestId(), e);
            KafkaMessageResponse response = new KafkaMessageResponse();
            response.setRequestId(request.getRequestId());
            response.setOperation("DELETE");
            response.setError(e.getMessage());
