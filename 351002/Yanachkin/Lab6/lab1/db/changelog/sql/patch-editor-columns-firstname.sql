...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'distcomp' AND table_name = 'tbl_editor' AND column_name = 'role'
  ) THEN
    ALTER TABLE distcomp.tbl_editor ADD COLUMN role VARCHAR(16) NOT NULL DEFAULT 'CUSTOMER';
  END IF;
END $$;
