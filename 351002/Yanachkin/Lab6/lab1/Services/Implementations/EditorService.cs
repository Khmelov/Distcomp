...
public class EditorService : IEditorService
{
    ...
    public EditorService(IEntityRepository<Editor> repository, IMapper mapper, IPasswordHasher<Editor> passwordHasher)
    {
    ...
    public async Task<EditorResponseTo> CreateAsync(EditorRequestTo request, CancellationToken cancellationToken = default)
    {
        ...
        editor.Id = 0;
        editor.Role = NormalizeRole(request.Role);
        editor.Password = _passwordHasher.HashPassword(editor, request.Password);
    ...
    private static string NormalizeRole(string? role)
    {
        ...
        if (normalized is not ("ADMIN" or "CUSTOMER"))
